...
>
> **Last updated:** 2026-06-26 (Code-audit pass — DRY, test coverage, apiFetch, _auth.js, Vite CVEs) ·
...
- **Test coverage:** significantly expanded by code-audit pass (2026-06-26). **130 tests** across 5
  test files: `scoring.test.js` (pure scoring + question health + malformed-input edge cases),
  `session.test.js` (localStorage session round-trips, unavailability handling), `db.test.js`
  (Firestore calls mocked via `vi.hoisted`, tests composite key construction, data shapes, legacy
  fallbacks, subscription mapping), `api/api-handlers.test.js` (`sanitize`, `buildDigest`,
  `buildSystemInstruction`, `buildContents`), and `src/components/components.test.jsx` (jsdom,
  EmptyState/Footer/Nav pure-render + stateful Nav tab/pill behaviour). Role-app integration tests
  remain the only untested area.
...
- **Counts (today):** 6 domains (shared, dept-neutral) · 9 competencies · 18 Pediatrics + 14
  ...
  + OB/GYN live**, 2 mockup) · **130** unit tests (5 test files) · **5** Firestore collections
