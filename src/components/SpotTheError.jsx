...
// Gemini with exactly one planted SOP violation). For each item they pick the
// message where the agent broke policy AND write a short explanation of why —
// click accuracy is the whole score (the explanation is shown back in the
// review, not graded). No correct/wrong feedback is shown during the run;
// results appear only on the review screen after the last item. Scores feed
// the capability matrix (like the main check).
//
...
//
// Flow:  loading → active (item-by-item: pick a message, explain why, Next —
//        no reveal) → review (all correct/missed feedback) → saving → done
//
...
export default function SpotTheError({
  ...
  const [picks,   setPicks]   = useState([]);   // [{ picked:number, correct:boolean, explanation:string }] aligned to items
  const [selected,    setSelected]    = useState(null); // current item's picked turn — changeable until Next commits it
  const [explanation, setExplanation] = useState('');   // current item's "why is this the error" text
  ...
  // The pick stays changeable (and unrevealed) until "Next" commits it along
  // with the navigator's explanation — correctness is only shown in the review.
  ...
  const canAdvance = selected !== null && explanation.trim().length > 0;
  ...
  const advance = () => {
    ...
    setPicks((prev) => {
      ...
      next[current] = {
        picked: selected,
        correct: selected === item.errorIndex,
          ...
          {items.map((item, i) => {
            ...
            return (
              <li key={i} className={`card spot-error__review-item spot-error__review-item--${ok ? 'ok' : 'miss'}`}>
                ...
                {!ok && pick?.picked !== undefined && (
                  <p className="spot-error__review-pick">
                    <strong>Your pick:</strong>{' '}
                    “{item.transcript[pick.picked]?.message}”
                  </p>
                )}
                ...
                {pick?.explanation && (
                  <div className="spot-error__review-yours">
                    <span className="spot-error__review-yours-label">Your reasoning</span>
                    <p className="spot-error__review-yours-text">{pick.explanation}</p>
                  </div>
                )}
              </li>
            );
  ...
  return (
    <section className="spot-error view-enter">
      ...
      <div className="card spot-error__header">
        <div className="spot-error__header-left">
          ...
          <p className="spot-error__instructions">
            ...
            <strong>Click the exact message where they went wrong, then explain why.</strong>{' '}
            You can change your pick until you continue — results are revealed after the last item.
          </p>
        </div>
      ...
      <div className="spot-error__transcript" role="list">
        {item.transcript.map((turn, i) => {
          ...
          const classes = [
            ...
            isAgent  ? 'spot-error__bubble--clickable' : '',
            isPicked ? 'spot-error__bubble--selected'  : '',
          ].filter(Boolean).join(' ');
          ...
          return (
            <div key={i} className={classes} role="listitem">
              ...
              <button
                ...
                disabled={!isAgent}
                type="button"
                aria-pressed={isAgent ? isPicked : undefined}
                aria-label={isAgent ? 'Select this message as the error' : undefined}
              >
                {turn.message}
              </button>
            </div>
      ...
      {/* Explain-your-pick panel — no correct/wrong verdict until the review */}
      {selected !== null && (
        <div className="card spot-error__feedback">
          <label className="spot-error__explain-label" htmlFor="spot-explain">
            Why is this the error?
            <span className="spot-error__explain-hint"> Briefly explain the policy the agent broke.</span>
          </label>
          <textarea
            id="spot-explain"
            ...
            placeholder="e.g. The agent shared results without verifying the caller's identity…"
            rows={3}
          />
          <button className="btn btn--primary" onClick={advance} type="button" disabled={!canAdvance}>
            {current < items.length - 1 ? 'Next item →' : 'Finish & see results'}
          </button>
        </div>
      )}
    </section>
