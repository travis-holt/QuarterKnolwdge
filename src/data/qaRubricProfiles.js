...
const OBGYN_GRADER_INSTRUCTIONS = [
  ...
  '- NAVIGATOR-VISIBLE CHART: when the scenario provides a NAVIGATOR-VISIBLE CHART (the simulated ECW chart the navigator was shown), judge every chart-dependent decision ONLY against those facts and against caller-provided information. A chart fact not listed in the navigator-visible chart was NOT visible to the navigator - never penalize the navigator (under any criterion) for failing to act on, discover, or infer it. HIDDEN CHART FACTS are grader ground-truth context only; a scored decision must never depend on a hidden fact the navigator could not see. When the correct outcome is "explain that no order/appointment is on file and route the clarification through the correct OB workflow", credit the navigator for doing exactly that against the navigator-visible chart.',
