...
//   * listen-gather  — measures CALLER-OBSERVABLE information gathering only. Not
//                      failed for an internal/system chart fact the navigator did
//                      not narrate; chart correctness lives in the knowledge/
//                      scheduling/workflow criteria (pilot fix 2026-07-24).
//   * sched-recap    — CONDITIONAL: applies only when the CORRECT workflow books
//                      an appointment during the call. When the correct outcome is
//                      no booking (clarification/TE/escalation/etc.) it is NA — a
//                      wrong booking is captured by sched-flow/know-rule, never by
//                      a second sched-recap penalty (pilot fix 2026-07-24).
  ...
  },
  {
    id: 'activeListening', name: 'Active Listening', criteria: [
      ...
      { id: 'listen-gather', points: 5, core: true, domainIds: ['intake', 'classification'], competencyIds: ['criticalThinking', 'communication'],
        text: 'Gathered the CALLER-PROVIDED information needed to handle the request before answering - identity, necessary clarification, symptoms or details, and preferences as the workflow requires - with no assumptions or premature answers. This measures CALLER-OBSERVABLE information gathering only. Do NOT mark NOT_MET merely because the navigator did not state or discover an internal/system chart fact; chart-state correctness belongs to the knowledge, scheduling, and workflow criteria once the simulated chart has actually made that fact available.' },
    ],
  },
  {
  ...
  },
  {
    id: 'scheduling', name: 'Appointment Scheduling', criteria: [
      ...
      { id: 'sched-recap', points: 7, core: false, domainIds: ['scheduling', 'documentation'], competencyIds: ['communication', 'sopApplication'],
        text: 'CONDITIONAL - applies ONLY when the CORRECT workflow for this call results in an appointment actually being booked/scheduled DURING this call. In that case the navigator recapped the appointment date, time, and place, and gave arrival instructions. If the correct outcome is NO booking (a clarification, a message/Telephone Encounter, an escalation, an approval that must be obtained first, a records/transfer step, or any other non-scheduling resolution), this is NA - there is no appointment to recap. Do NOT mark this NOT_MET for failing to recap an appointment that should not exist. If the navigator WRONGLY booked an appointment the correct workflow said not to book, that error belongs to sched-flow / know-rule / the applicable workflow criteria, not to a second penalty here.' },
    ],
  },
  {
...
const OBGYN_GRADER_INSTRUCTIONS = [
  ...
  '- ACTIVE LISTENING: [listen-ack] is satisfied by natural recognition of the request ("sure, I can help you schedule that", "okay, let me check your chart", a concise recap, or a clarifying question). Do not require emotional scripting on a routine call. [listen-gather] measures whether the navigator gathered the CALLER-PROVIDED / caller-observable information needed to handle the request (identity, necessary clarification, symptoms/details, preferences). Caller-volunteered information counts as gathered: when gestational age, pregnancy status, reason for visit, symptoms, preferences, callback information, or another ordinary workflow fact is clear, do NOT require a redundant question solely because the scenario says ask/gather/obtain/establish/collect/confirm, unless the SOP specifically requires re-confirmation. It stays strict when the navigator genuinely fails to obtain information the request needs. Repeating information the caller already clearly supplied may itself support a criterion-specific [listen-ack] / Active Listening miss when it shows the navigator did not listen. But do NOT mark [listen-gather] NOT_MET merely because the navigator did not state, discover, or narrate an internal/system chart fact - chart-state correctness is scored under the knowledge, scheduling, and workflow criteria, once the NAVIGATOR-VISIBLE CHART has actually made that fact available. Do not double-penalize one invisible-chart issue as an active-listening failure.',
  '- SCHEDULING RECAP: [sched-recap] is CONDITIONAL and applies ONLY when the CORRECT workflow for this call books an appointment DURING the call. When the correct outcome is no booking - a clarification, a message/Telephone Encounter, an escalation, an approval-first step, a records/transfer step, or any other non-scheduling resolution - mark [sched-recap] NA; there is nothing to recap. Never deduct [sched-recap] for failing to recap an appointment that should not have been created. If the navigator WRONGLY booked when the correct workflow was not to book, capture that under [sched-flow] / [know-rule] / the applicable workflow criteria - do NOT add a separate [sched-recap] penalty for an appointment that should never have existed.',
  '- NAVIGATOR-VISIBLE CHART: when the scenario provides a NAVIGATOR-VISIBLE CHART (the simulated ECW chart the navigator was shown), that chart is the COMPLETE record of what the navigator could see. Judge every chart-dependent decision ONLY against those facts and against caller-provided information. A chart section listed as "None on file" IS a fact the navigator could see and act on. A chart section that is NOT listed was never shown to the navigator - do not assume what it contains, and never penalize the navigator (under any criterion) for failing to act on, discover, or infer it. When the correct outcome is "explain that no order/appointment is on file and route the clarification through the correct OB workflow", credit the navigator for doing exactly that against the navigator-visible chart.',
  ...
  '- DOCUMENTATION: judge [doc-reason] against OB/GYN documentation conventions. Pediatric examples (physical-exam/PE status, newborn abbreviations) are NOT OB/GYN standards and must never be required here. Do not fail documentation merely because a caller-volunteered workflow fact was not redundantly asked; a documentation miss needs its own applicable routing, transfer, or record-specific basis.',
].join('\n');
