...
//                 script?:  [{ say, not, why }],          // exact-phrasing pairs
//                 example?: { intro?, turns: [{ speaker: 'caller'|'nav',
//                                               text, note? }] },
//                 doc?:     { label, lines: string[], note? } }], // model document
//     mistakes?: [{ mistake, consequence, instead }],
//     quickRef?: { title, rows: [{ label, value }] },
//     drill?:    [{ prompt, options: [{ text, correct?, why }] }],
//     simulations?: [{                                    // branching call sims
//       label, title, intro, callerName, start,          // label = dept toggle
...
export const TRAINING_MODULES = [
  // ── INTAKE ────────────────────────────────────────────────────────────────
  {
    ...
    lessons: [
      {
        ...
        points: [
          'Pediatrics: ask for the PARENT\'S phone number first — it pulls up the whole family at once, and parents routinely call about two or three children in one call.',
          'Adult departments (OB/GYN, Behavioral Health, Internal Medicine): the patient usually calls for herself — verify date of birth first, then confirm first and last name per privacy policy, and confirm the correct account before discussing anything or opening a TE.',
      ...
      },
      {
        ...
        points: [
          ...
          'Pediatrics multi-child calls: work each child\'s request inside that child\'s own chart. Never batch siblings\' requests under whichever chart happens to be open.',
          'OB/GYN especially: once the account is verified, check the chart (Encounters, Medical Summary, last note, open TEs) before asking the patient to explain everything — the chart often already says what she needs. Don\'t schedule from her wording alone.',
        ],
        example: {
    ...
    quickRef: {
      ...
      rows: [
        { label: 'Pediatrics', value: 'Parent\'s phone number first → family account → pick the child → confirm DOB' },
        { label: 'OB/GYN · BH · Internal Med', value: 'DOB first → confirm first + last name → confirm account before discussing anything' },
        { label: 'Same-name collision', value: 'DOB, then address/phone — before opening either chart' },
        { label: 'OB/GYN, after verifying', value: 'Check Encounters / Medical Summary / last note / open TEs before asking her to re-explain' },
        { label: 'Yellow "Y" on a Peds pull-up', value: 'Coverage active, but Aizer is NOT the PCP' },
    ...
    simulations: [
      {
        ...
        nodes: {
          ...
          n3: {
            ...
            choices: [
              { text: 'A sick visit only books for the same day — but I can book it as a same-day visit tomorrow morning, or find a slot today. Which works?', next: 'end_strong', tone: 'good', feedback: 'Same-day rule held, and you offered a real path instead of a flat no.' },
          ...
          end_sameday: { ending: { verdict: 'mixed', title: 'Good ID, wrong visit type', summary: 'Your identity work was clean, but a same-day sick visit booked for tomorrow is invalid by definition and will be unwound — the child still isn\'t on the schedule.', lesson: 'Same-day sick = the day itself only. If tomorrow is better, book it as tomorrow\'s same-day slot.' } },
        },
      },
      {
        label: 'OB-GYN',
    ...
    keyTakeaways: [
      'Peds = parent phone first; adult departments = DOB first, then name, then confirm the account.',
      'Never open or discuss a chart until identity is confirmed.',
      'OB/GYN: read the chart before you book — the record, not the patient\'s wording, decides the visit.',
    ],
  },
  ...
  // ── CLASSIFICATION ────────────────────────────────────────────────────────
  {
    ...
    lessons: [
      {
        ...
        points: [
          'Every request maps to a workflow: scheduling, symptom / clinical question, refill, labs, procedure, MFM, transfer, late arrival, or callback. If you can\'t name it, you can\'t route it.',
    ...
    quickRef: {
      ...
      rows: [
        { label: '"Is this normal?" / "Are my results okay?"', value: 'Clinical question → TE to the clinical team (OB Portal). Never answered by a navigator.' },
        { label: 'Serious OB symptom (↓ movement · heavy bleeding · severe pain · possible loss / water breaking)', value: 'Gather facts (no triage) → High Priority TE to OB Portal → OB Urgent Calls channel → follow clinical team. Navigators never dispatch or self-triage.' },
        { label: 'Positive test, reliable LMP', value: 'Scheduling — build the New OB pair (8–12 weeks)' },
        { label: 'Positive test, unknown / unreliable LMP', value: 'Scheduling — Confirmation of Pregnancy visit (15 min)' },
  ...
  // ── ROUTING ───────────────────────────────────────────────────────────────
  {
    ...
    lessons: [
      ...
      },
      {
        ...
        points: [
          ...
          'Patient completely out of medication (Peds)? The refill TE is marked HIGH PRIORITY — the difference between today and "in a few days."',
          'Serious OB symptoms — heavy bleeding, decreased fetal movement, severe pain, possible miscarriage, possible water breaking — go to the clinical team, NOT to Labor & Delivery on your say-so: gather facts, High Priority TE to OB Portal, message the "OB Urgent Calls" Intermedia channel, follow their direction.',
          'An open OB Urgent or GYN Urgent slot is NOT authorization. A navigator books it only after written nurse/provider approval — never because the slot is empty and the patient sounds urgent.',
        ],
        script: [
          {
    ...
    quickRef: {
      ...
      rows: [
        { label: 'Peds: medical Q / labs / refills', value: 'TE → PEDS Encounters queue (+ HIGH PRIORITY if completely out)' },
        ...
        { label: 'OB serious symptoms (bleeding · ↓ movement · severe pain · possible loss)', value: 'Gather → High Priority TE to OB Portal → OB Urgent Calls channel → follow clinical team. Never dispatch or book an urgent slot yourself.' },
      ],
    },
    drill: [
      {
  ...
  // ── SCHEDULING ────────────────────────────────────────────────────────────
  {
    ...
    lessons: [
      {
        ...
        points: [
          'OB/GYN: "checkup," "ultrasound," "the doctor told me" are not orders. The order lives in the chart — the Medical Summary RTO, the last provider note, or a TE. Follow it; don\'t recalculate cadence from memory.',
          'RTO reading: "RTO 4 weeks" = a routine MD visit. "RTO 3 weeks BPP w/MD" = a BPP sonogram PLUS an MD visit. If the patient claims a scan or interval the chart doesn\'t support, send a TE to OB Portal to clarify — don\'t create it from her statement.',
          'Pediatrics timing still holds: physicals follow the cadence and the "one calendar year plus one day" commercial rule; the Fidelis/Medicaid early-PE exception needs BOTH a birthday since the last PE AND at least six months elapsed.',
        ],
        script: [
          {
    ...
    quickRef: {
      ...
      rows: [
        { label: 'OB order source', value: 'Medical Summary RTO / last note / TE — never the patient\'s wording' },
        { label: 'RTO 4 weeks', value: 'Routine MD visit' },
        { label: 'RTO 3 weeks BPP w/MD', value: 'BPP sonogram + MD visit, paired' },
        { label: 'New OB (first prenatal)', value: '30-min sono + 30-min provider, back-to-back, same day · 2nd record OB Verified' },
        { label: 'Positive test', value: 'Reliable LMP → New OB pair · unknown/unreliable LMP → Confirmation of Pregnancy (15 min)' },
        { label: 'Anatomy scan', value: '1-hr sono, Dr. Rosenberg as SCAN provider + MD visit — NOT an MFM appointment' },
        { label: 'Annual GYN "UTD"', value: 'Actual Annual GYN in-department within a year (Pap-only / outside / postpartum don\'t count)' },
        { label: 'Open OB/GYN Urgent slot', value: 'NOT authorization — book only with written nurse/provider approval' },
        { label: 'Peds same-day sick / Fidelis early PE', value: 'Same day only · early PE needs birthday AND ≥6 months' },
      ],
    },
    drill: [
      {
    ...
    keyTakeaways: [
      'OB/GYN: schedule from the chart (Medical Summary RTO / note / TE), never from the patient\'s wording.',
      'New OB = 30-min sono + 30-min provider, back-to-back, same day, OB Verified on the second; paired appointments never split.',
      'An open urgent slot isn\'t authorization; Peds early PE needs birthday AND ≥6 months.',
    ],
  },
  ...
  // ── BOUNDARIES ────────────────────────────────────────────────────────────
  {
    ...
    lessons: [
      ...
      },
      {
        ...
        points: [
          ...
          'On a reported pregnancy loss or miscarriage: escalate (High Priority TE to OB Portal + urgent channel) and await clinical direction. Do NOT independently cancel future appointments, remove the pregnancy, or decide follow-up care.',
    ...
    quickRef: {
      ...
      rows: [
        ...
        { label: 'Cancel OB appts on a reported loss', value: 'High Priority TE + urgent channel, await clinical direction' },
  ...
  // ── DOCUMENTATION ─────────────────────────────────────────────────────────
  {
    ...
    lessons: [
      {
        ...
        points: [
          'A complete refill TE answers every question the clinical team would otherwise call back to ask: medication + dosage, prescribing provider (from the e-prescription log), preferred pharmacy, best callback number, and (Peds) whether the patient is OUT → HIGH PRIORITY.',
          'Use a short, recognizable reason: "Severe Pelvic Pain," "Decreased Fetal Movement," "Refill Request," "Missing BPP Order." Put the detail in the TE body — don\'t write a diagnosis.',
          'Never type "urgent" into the message to convey priority. Use the High Priority checkbox and, for serious OB symptoms, the OB Urgent Calls channel — that\'s the workflow.',
          'The DESTINATION is part of the documentation — a perfect TE in the wrong queue helps no one. And for paired appointments, mark the second record "OB Verified."',
        ],
        doc: {
      ...
      },
      {
        ...
        points: [
          ...
          'An open TE for a DIFFERENT issue? Create a separate TE. Never fold an unrelated refill into an open bleeding TE.',
          'Same-day sick visits (Peds): ALL reported symptoms go in the reason field — "FEVER + COUGH since last night," not just "sick." The provider preps from that line.',
        ...
        script: [
          {
            say: '"I see an open note about your bleeding from yesterday — I\'ll add an update to that one rather than start a new one, and keep it High Priority."',
    ...
    mistakes: [
      ...
      },
      {
        ...
        instead: 'Use the High Priority checkbox, and for serious OB symptoms message the OB Urgent Calls channel.',
      },
    ],
    quickRef: {
      title: 'Documentation — pin this',
      rows: [
        { label: 'Refill TE fields', value: 'Medication · prescriber (e-Rx log) · pharmacy · callback · (Peds) out? → High Priority' },
        { label: 'Reason field', value: 'Short + recognizable ("Severe Pelvic Pain," "Refill Request") — never a diagnosis' },
        { label: 'Priority', value: 'High Priority checkbox (+ OB Urgent Calls channel if serious) — never type "urgent"' },
        ...
        { label: 'Peds same-day sick', value: 'All symptoms in the reason field ("FEVER + RASH since Tue")' },
        { label: 'Paired appointment', value: 'Mark the second record OB Verified' },
      ],
    },
    drill: [
      {
    ...
    keyTakeaways: [
      ...
      'High Priority is a checkbox (+ the OB Urgent Calls channel for serious symptoms) — never a word you type.',
    ],
  },
];
