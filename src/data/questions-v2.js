...
const PEDS = [
  ...
  // ── routing ────────────────────────────────────────────────────────────────
  {
    ...
    options: [
      { id: 'a', text: 'To Marisa Kraft, who owns shots and immunizations.', points: 15,
        rationale: 'Marisa owns immunizations and imaging, not controlled-substance follow-ups.' },
      ...
      { id: 'c', text: 'Send a TE to Sally Carilli, the controlled-substance follow-up owner.', points: 100,
        rationale: 'Correct per SOP: controlled-substance follow-ups (Concerta is on the list) go to Sally Carilli via a TE.' },
      { id: 'd', text: 'To Anisa Azeez, who owns referrals.', points: 10,
        rationale: 'Anisa owns referral-related TEs, not controlled substances.' },
    ],
    correctOptionId: 'c',
  },
  {
    id: 'qv2-peds-rt-2',
    domainId: 'routing',
    competencies: ['sopKnowledge', 'sopApplication', 'problemResolution'],
    scenario:
      'A parent calls to get their child’s shots done. The child’s physical exam (PE) is up to date. How should you handle it?',
    options: [
      { id: 'a', text: 'Explain that shots are not scheduled as appointments; since the PE is up to date, send a TE to PEDS Marisa Kraft with the parent’s call-back number and confirmation the PE is up to date.', points: 100,
        rationale: 'Correct per SOP: immunizations are not booked as visits — with the PE up to date, route a TE to Marisa Kraft.' },
      { id: 'b', text: 'Book the immunization into an open nursing slot for today.', points: 5,
        rationale: 'Per SOP, shots, immunizations, and vaccinations are not scheduled as appointments.' },
      { id: 'c', text: 'Send the TE to the PEDS Encounters queue like a medical question.', points: 30,
        rationale: 'Right mechanism (a TE), wrong owner — immunizations route to Marisa Kraft, not the general encounters queue.' },
      { id: 'd', text: 'Route it to Sally Carilli.', points: 10,
        rationale: 'Sally owns controlled-substance follow-ups, not immunizations.' },
    ],
    correctOptionId: 'a',
  },
  {
  ...
  // ── documentation ────────────────────────────────────────────────────────────
  {
    ...
    scenario:
      'A parent calls asking about their child’s recent lab results. You are sending the Telephone Encounter to the PEDS Encounters queue so a nurse calls back. Which handling is complete?',
    options: [
      { id: 'a', text: 'Check existing TEs for any provider or nurse instructions first, then send the TE to the PEDS Encounters queue with the parent’s best call-back number and which lab(s) they asked about.', points: 100,
        rationale: 'Correct per SOP: always check existing TEs first, then route a complete TE (call-back number + which labs) for a nurse to call back.' },
      { id: 'b', text: 'Send "parent wants results, please call" with no call-back number or lab detail.', points: 5,
        rationale: 'Incomplete — the nurse has no call-back number and no idea which labs, forcing a second round-trip.' },
      { id: 'c', text: 'Send the TE with the call-back number, but skip checking existing TEs before creating a new one.', points: 40,
        rationale: 'Nearly right, but the SOP requires checking existing TEs first — a provider may have already left instructions.' },
      { id: 'd', text: 'Read the results to the parent yourself and note that you told them.', points: 0,
        rationale: 'Navigators never convey lab results (and a black lock forbids it) — a scope violation, not documentation.' },
    ],
    correctOptionId: 'a',
  },
  {
